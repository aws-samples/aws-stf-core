...
export interface ScorpioDatabaseProps{
    vpc: Vpc
    secret_arn: string
}

export class ScorpioDatabase extends Construct{

    public readonly database_endpoint: string
    public readonly database_port: string
    public readonly sg_database: SecurityGroup

    constructor(scope: Construct, id: string, props: ScorpioDatabaseProps) {
        super(scope, id)
       ...
       if(Parameters.stf_scorpio.aurora_serverless){
        const aurora_cluster = new ServerlessCluster(this, 'AuroraServerlessCluster', {
            ...
            vpc: props.vpc, 
            defaultDatabaseName: Parameters.stf_scorpio.dabaseName,
        ...
        const database = new DatabaseInstance(this, 'DatabaseInstance', {
            ...
            vpc: props.vpc, 
            securityGroups: [sg_database],
            databaseName: Parameters.stf_scorpio.dabaseName,
            vpcSubnets:{
                subnetType: SubnetType.PRIVATE_ISOLATED
            }
        })
